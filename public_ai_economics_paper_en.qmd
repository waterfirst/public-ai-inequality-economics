...
This is no longer a hypothetical policy. In July 2026 the Korean Ministry of Science and ICT launched the "AI for All" (*모두의 AI*) project, a plan to deliver a **free, usage-unlimited domestic AI chatbot to the entire population within the year**, bundled with a public-service AI agent that finds and files eligible public benefits on a citizen's behalf. The stated motivation is precisely an AI-usage gap: two-thirds of Koreans have used AI and generative-AI users reached 23 million in 2025, yet **roughly one-third of the population still does not use AI at all**, and much of the rest depends on the free tiers of foreign services. This is the exact policy object the present model analyzes—universal nominal access delivered through a sovereign, multi-model architecture. Our central result speaks directly to it: a free universal chatbot removes the first-order access gap but, on a realistic Korean population, does not by itself reverse polarization; the non-using third that motivates the program is also the low-education, non-employed mass for which access alone yields negative inequality-adjusted welfare in the model. Whether *모두의 AI* narrows or widens distributional gaps therefore depends on the compensatory design—education, care, and broadened AI-capital ownership—studied below, not on the launch of the chatbot itself.
...
Here $\lambda$ is social diffusion and $\sigma$ is peer learning. Homophily can produce capability clusters: the same spillover that accelerates national adoption may amplify between-cluster inequality when disadvantaged agents mainly connect to other low-skill agents. Progressive education can instead seed high-return nodes inside those clusters. The browser laboratory ([live simulator](https://waterfirst.github.io/public-ai-inequality-economics/)) exposes these mechanisms and visualizes the resulting network; the publication model treats them as hypotheses pending Korean network calibration.
...
- **Interactive simulator (live):** <https://waterfirst.github.io/public-ai-inequality-economics/> — a browser-based Korea Policy Lab (React + Three.js) that runs the policy comparison, lets the reader vary parameters and seeds, load the NVIDIA aggregate profile, and export CSV/JSON. Provenance and limitations are shown next to the upload control; heavy Monte Carlo results are precomputed in the repository.
- **Source repository:** <https://github.com/waterfirst/public-ai-inequality-economics>
- Research contract and permitted claims: `research-contract.md`
...
- Korean official-statistics download (KOSIS/ECOS OpenAPI): `scripts/10_download_kr_data.py`
- Parameter calibration from real data: `scripts/11_calibrate_from_kr_data.py`
- Real-population simulation (NVIDIA × KOSIS × ECOS): `scripts/12_public_ai_real_simulation.py`
- Publication-grade robustness (paired MC, ablations, finite size): `scripts/13_rigor_experiments.py`
...
- Real-population and robustness outputs: `results/public_ai_real/`, `results/rigor/`
