...
for t in [
 ...
 "분석의 방법론적 핵심은 세 가지다. 첫째, 모든 정책 비교를 동일 초기 인구와 동일 난수열을 공유하는 짝비교(common random numbers)로 수행하여 정책 간 차이가 표본 변동이 아니라 정책 설계에서 비롯되도록 하였다. 둘째, 불평등(Gini·Atkinson)과 양극화(Wolfson·중간층 비중)를 구분하여 함께 보고함으로써 ‘중간층의 공동화’를 포착하고, 효율 측면에서는 총생산과 함께 균등분배 대등소득(EDE)을 보고하여 ‘평균은 오르지만 후생은 나빠지는’ 상황을 식별하였다. 셋째, 인구 자료 처리에 개인 이름·서사 필드를 저장하지 않는 데이터 거버넌스 원칙을 적용하고, 집계 산출물이 사전 정의된 스키마와 금지 필드 규칙을 통과하는지 자동 검증하였다.",
 "종합하면 본 연구는 ‘무료 보급이 곧 격차 해소’라는 통념을 산업통계에 근거해 반증한다. 실제 한국 인구에서 무료 접근은 접근 격차를 줄이지만 양극화를 되돌리지 못하며, 특히 접근만으로는 순후생이 음(−)이 되는 저학력·비경제활동 계층이 존재한다. 분배를 개선하는 유일한 조합은 접근에 더해 저역량층 우선 교육, 돌봄·행정 부담 경감, 그리고 시민·근로자의 AI 자본 청구권 확대를 결합한 종합안이었다. 이때 양극화 완화는 자본소유 환류가, 취약계층 고용 개선은 교육·돌봄이 이끄는 ‘두 채널 구조’가 확인되었다.",
